/*:
 ...
 First let's talk about what do we mainly have available on the market:
 ...
 * The combustion engine that powers the car;
 * The transmission that converts the power coming from the engine to momentum which drives the wheels;
 * The differential that allows your wheels to rotate in different speeds so the car can make turns more easily.
 ...
 * The battery pack that is a set of identical batteries that store charge so the car can turn on;
 * The differential that has the same functionality of the combustion engine one but it's attatched to the motor.

 Different from the combustion engine cars, electric cars have a much more power efficient design and don't rely on non-renewable energy.
